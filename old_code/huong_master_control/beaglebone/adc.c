...
int main() {
	...
	// ain = fopen("/sys/devices/platform/bone_capemgr/slots", "w"); // use this for micro sd card
	// ain = fopen("/sys/devices/bone_capemgr.8/slots", "w"); // use this for emmc boot	
	ain = fopen("/sys/devices/bone_capemgr.9/slots", "w"); // use this for older version of debian on bot's micro sd card
	fseek(ain, 0, SEEK_SET);

	fprintf(ain, "BB-ADC"); // use this for micro sd card
	// fprintf(ain, "cape-bone-iio"); // use this for emmc boot 
